...
const TaskCard = ({ task, onUpdate, onDelete, tags }: TaskCardProps) => {
  ...
  const handleMenuToggle = () => {
    if (!showMenu && buttonRef.current) {
      ...
      let left = rect.right - 192 // 192px is menu width (w-48)
      ...
      if (top + 100 > viewportHeight) { // 100px is approximate menu height
        top = rect.top - 100 - 8
      }
  ...
  return (
    <>
      ...
      {showMenu && (
        ...
        >
          <div 
            className="absolute w-48 bg-white dark:bg-gray-800 rounded-xl shadow-soft dark:shadow-soft-dark border border-gray-200 dark:border-gray-700 overflow-hidden"
            style={{
